...
func (c *cleanupTask) Run(ctx context.Context) error {
	rows, err := c.devices.Cleanup(ctx, time.Now().Add(-c.config.MaxAge))
	if err != nil {
		return fmt.Errorf("failed to cleanup devices: %w", err)
	}

	if rows > 0 {
		c.logger.Info("cleaned up devices", zap.Int64("rows", rows))
	}
